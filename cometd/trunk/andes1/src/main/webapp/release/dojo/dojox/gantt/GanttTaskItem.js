...
if (!dojo._hasResource["dojox.gantt.GanttTaskItem"]) {
	...
	dojo.declare("dojox.gantt.GanttTaskControl", null, {constructor:function (taskInfo, project, chart) {
		...
		var lineVerticalLeft = dojo.create("div", {innerHTML:"&nbsp;", className:"ganttTaskLineVerticalLeft"}, this.ganttChart.panelNames.firstChild);
		var cTaskName = this.cTaskNameItem[0], pcTaskName = this.parentTask.cTaskNameItem[0];
		dojo.style(lineVerticalLeft, {height:(cTaskName.offsetTop - pcTaskName.offsetTop) + "px", top:(pcTaskName.offsetTop + 5) + "px", left:(pcTaskName.offsetLeft - 9) + "px"});
		var LineHorizontalLeft = dojo.create("div", {noShade:true, color:"#000000", className:"ganttTaskLineHorizontalLeft"}, this.ganttChart.panelNames.firstChild);
		dojo.style(LineHorizontalLeft, {left:(pcTaskName.offsetLeft - 9) + "px", top:(cTaskName.offsetTop + 5) + "px", height:"1px", width:(cTaskName.offsetLeft - pcTaskName.offsetLeft + 4) + "px"});
		...
		var arrowImg = dojo.create("div", {className:"ganttImageArrow"});
		...
		if (posYPreviousTask < posYChildTask) {
			...
			dojo.style(lineVerticalRight, {height:(posYChildTask - this.ganttChart.heightTaskItem / 2 - posYPreviousTask - 3) + "px", width:"1px", left:(posXPreviousTask + widthPreviousTask - 20) + "px", top:(posYPreviousTask + this.ganttChart.heightTaskItem) + "px"});
			dojo.addClass(lineHorizontal, "ganttTaskLineHorizontal");
			dojo.style(lineHorizontal, {width:(15 + (posXChildTask - (widthPreviousTask + posXPreviousTask))) + "px", left:(posXPreviousTask + widthPreviousTask - 20) + "px", top:(posYChildTask + 2) + "px"});
			dojo.addClass(arrowImg, "ganttTaskArrowImg");
			dojo.style(arrowImg, {left:(posXChildTask - 7) + "px", top:(posYChildTask - 1) + "px"});
		} else {
			dojo.addClass(lineVerticalRight, "ganttTaskLineVerticalRightPlus");
			dojo.style(lineVerticalRight, {height:(posYPreviousTask + 2 - posYChildTask) + "px", width:"1px", left:(posXPreviousTask + widthPreviousTask - 20) + "px", top:(posYChildTask + 2) + "px"});
		...
		if (isOpen) {
			for (var i = 0; i < task.childTask.length; i++) {
				var cTask = task.childTask[i], cTaskItem0 = cTask.cTaskItem[0], cTaskName0 = cTask.cTaskNameItem[0], cTaskItem1 = cTask.cTaskItem[1], cTaskName1 = cTask.cTaskNameItem[1], cTaskItem2 = cTask.cTaskItem[2], cTaskName2 = cTask.cTaskNameItem[2];
		...
		var taskItem0 = task.cTaskItem[0], taskName0 = task.cTaskNameItem[0], taskItem1 = task.cTaskItem[1], taskName1 = task.cTaskNameItem[1], taskItem2 = task.cTaskItem[2], taskName2 = task.cTaskNameItem[2];
		...
		var itemControl = dojo.create("div", {id:this.taskItem.id, className:"ganttTaskItemControl"});
		dojo.style(itemControl, {left:this.posX + "px", top:this.posY + "px"});
		var divTaskItem = dojo.create("div", {className:"ganttTaskDivTaskItem"}, itemControl);
		var tblTaskItem = dojo.create("table", {cellPadding:"0", cellSpacing:"0", width:this.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px", className:"ganttTaskTblTaskItem"}, divTaskItem);
		var rowTblTask = tblTaskItem.insertRow(tblTaskItem.rows.length);
		if (this.taskItem.percentage != 0) {
			var cellTblTask = dojo.create("td", {height:this.ganttChart.heightTaskItem + "px", width:this.taskItem.percentage + "%"}, rowTblTask);
			cellTblTask.style.lineHeight = "1px";
			var imageProgress = dojo.create("div", {className:"ganttImageTaskProgressFilled"}, cellTblTask);
			dojo.style(imageProgress, {width:(this.taskItem.percentage * this.taskItem.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
		}
		if (this.taskItem.percentage != 100) {
			var cellTblTask = dojo.create("td", {height:this.ganttChart.heightTaskItem + "px", width:(100 - this.taskItem.percentage) + "%"}, rowTblTask);
			cellTblTask.style.lineHeight = "1px";
			var imageProgressFill = dojo.create("div", {className:"ganttImageTaskProgressBg"}, cellTblTask);
			dojo.style(imageProgressFill, {width:((100 - this.taskItem.percentage) * this.taskItem.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
		}
		if (this.ganttChart.isContentEditable) {
			var divTaskInfo = dojo.create("div", {className:"ganttTaskDivTaskInfo"}, itemControl);
			var tblTaskInfo = dojo.create("table", {cellPadding:"0", cellSpacing:"0", height:this.ganttChart.heightTaskItem + "px", width:this.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px"}, divTaskInfo);
			var rowTaskInfo = tblTaskInfo.insertRow(0);
			var cellTaskInfo = dojo.create("td", {align:"center", vAlign:"top", height:this.ganttChart.heightTaskItem + "px", className:"ganttMoveInfo"}, rowTaskInfo);
			var divTaskName = dojo.create("div", {className:"ganttTaskDivTaskName"}, itemControl);
			var divMove = dojo.create("div", {}, divTaskName);
			dojo.create("input", {className:"ganttTaskDivMoveInput", type:"text"}, divMove);
			dojo.isIE && dojo.style(divMove, {background:"#000000", filter:"alpha(opacity=0)"});
			dojo.style(divMove, {height:this.ganttChart.heightTaskItem + "px", width:this.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px"});
			var divResize = dojo.create("div", {className:"ganttTaskDivResize"}, divTaskName);
			dojo.create("input", {className:"ganttTaskDivResizeInput", type:"text"}, divResize);
			dojo.style(divResize, {left:(this.taskItem.duration * this.ganttChart.pixelsPerWorkHour - 10) + "px", height:this.ganttChart.heightTaskItem + "px", width:"10px"});
			this.ganttChart._events.push(dojo.connect(divMove, "onmousedown", this, function (event) {
			...
			this.ganttChart._events.push(dojo.connect(divMove, "onmouseover", this, function (event) {
				event.target && (event.target.style.cursor = "move");
			}));
			this.ganttChart._events.push(dojo.connect(divMove, "onmouseout", this, function (event) {
				event.target.style.cursor = "";
			}));
			this.ganttChart._events.push(dojo.connect(divResize, "onmousedown", this, function (event) {
			...
			this.ganttChart._events.push(dojo.connect(divResize, "onmouseover", this, function (event) {
				(!this.ganttChart.isMoving) && (!this.ganttChart.isResizing) && event.target && (event.target.style.cursor = "e-resize");
			}));
			this.ganttChart._events.push(dojo.connect(divResize, "onmouseout", this, function (event) {
				!this.checkResize && event.target && (event.target.style.cursor = "");
			}));
		}
		return itemControl;
	}, createTaskNameItem:function () {
		var divName = dojo.create("div", {id:this.taskItem.id, className:"ganttTaskTaskNameItem", title:this.taskItem.name + ", id: " + this.taskItem.id + " ", innerHTML:this.taskItem.name});
		dojo.style(divName, "top", this.posY + "px");
		dojo.attr(divName, "tabIndex", 0);
		if (this.ganttChart.isShowConMenu) {
			this.ganttChart._events.push(dojo.connect(divName, "onmouseover", this, function (event) {
			...
			this.ganttChart._events.push(dojo.connect(divName, "onkeydown", this, function (event) {
			...
			this.ganttChart._events.push(dojo.connect(divName, "onmouseout", this, function () {
			...
			this.ganttChart._events.push(dojo.connect(this.ganttChart.tabMenu.menuPanel, "onkeydown", this, function (event) {
				if (this.ganttChart.tabMenu.isShow && event.keyCode == dojo.keys.ESCAPE) {
					this.ganttChart.tabMenu.hide();
				}
			}));
		...
		var divDesc = dojo.create("div", {innerHTML:this.objKeyToStr(this.getTaskOwner()), className:"ganttTaskDescTask"});
		dojo.style(divDesc, {left:posX + "px", top:this.posY + "px"});
		...
		this.posX = this.ganttChart.getPosOnDate(this.taskItem.startTime);
		dojo.style(itemControl, {"left":this.posX + "px"});
		...
		if (this.taskItem.percentage != 0) {
			...
			dojo.style(imageProgress, {width:(!this.taskItem.duration ? 1 : (this.taskItem.percentage * this.taskItem.duration * this.ganttChart.pixelsPerWorkHour / 100)) + "px", height:this.ganttChart.heightTaskItem + "px"});
		}
		if (this.taskItem.percentage != 100) {
			...
			dojo.style(imageProgressFill, {width:(!this.taskItem.duration ? 1 : ((100 - this.taskItem.percentage) * this.taskItem.duration * this.ganttChart.pixelsPerWorkHour / 100)) + "px", height:this.ganttChart.heightTaskItem + "px"});
		}
		if (this.ganttChart.isContentEditable) {
			...
			dojo.style(divResize, {"left":(this.taskItem.duration * this.ganttChart.pixelsPerWorkHour - 10) + "px"});
		...
		var posX = (this.posX + this.taskItem.duration * this.ganttChart.pixelsPerWorkHour + 10);
		dojo.style(divDesc, {"left":posX + "px"});
		return divDesc;
		...
		var widthPreviousTask = parseInt(this.predTask.cTaskItem[0].firstChild.firstChild.width);
		if (posYPreviousTask < posYChildTask) {
			dojo.style(lineVerticalRight, {"height":(posYChildTask - this.ganttChart.heightTaskItem / 2 - posYPreviousTask - 3) + "px", "left":(posXPreviousTask + widthPreviousTask - 20) + "px"});
			dojo.style(lineHorizontal, {"width":(15 + (posXChildTask - (widthPreviousTask + posXPreviousTask))) + "px", "left":(posXPreviousTask + widthPreviousTask - 20) + "px"});
			dojo.style(arrowImg, {"left":(posXChildTask - 7) + "px"});
		} else {
			dojo.style(lineVerticalRight, {"height":(posYPreviousTask + 2 - posYChildTask) + "px", "left":(posXPreviousTask + widthPreviousTask - 20) + "px"});
			dojo.style(lineHorizontal, {"width":(15 + (posXChildTask - (widthPreviousTask + posXPreviousTask))) + "px", "left":(posXPreviousTask + widthPreviousTask - 20) + "px"});
			dojo.style(arrowImg, {"left":(posXChildTask - 7) + "px"});
		}
		return arrLines;
		...
		if (parentTask) {
			...
			if (this.taskItem.previousChildTask) {
				...
				this.posY = dojo.style(lastChildTask.cTaskItem[0], "top") + this.ganttChart.heightTaskItem + this.ganttChart.heightTaskItemExtra;
				this.previousChildTask.nextChildTask = this;
			} else {
				this.posY = dojo.style(task.cTaskItem[0], "top") + this.ganttChart.heightTaskItem + this.ganttChart.heightTaskItemExtra;
			}
		...
		var treeImg = dojo.create("div", {id:this.taskItem.id, className:"ganttImageTreeCollapse"});
		dojo.attr(treeImg, "tabIndex", 0);
		dojo.forEach(["onclick", "onkeydown"], function (e) {
			this.ganttChart._events.push(dojo.connect(treeImg, e, this, function (evt) {
		...
		dojo.style(treeImg, {left:(dojo.style(this.cTaskNameItem[0], "left") - 12) + "px", top:(dojo.style(this.cTaskNameItem[0], "top") + 3) + "px"});
		return treeImg;
		...
		if ((percentage != 0) && (percentage != 100)) {
			if ((this.taskItem.percentage != 0) && (this.taskItem.percentage != 100)) {
				...
				if ((this.taskItem.percentage == 0) || (this.taskItem.percentage == 100)) {
					...
					var cellTblTask = dojo.create("td", {height:this.ganttChart.heightTaskItem + "px", width:percentage + "%"}, trow);
					cellTblTask.style.lineHeight = "1px";
					var imageProgressFill = dojo.create("div", {className:"ganttImageTaskProgressFilled"}, cellTblTask);
					dojo.style(imageProgressFill, {width:(percentage * this.taskItem.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
					cellTblTask = dojo.create("td", {height:this.ganttChart.heightTaskItem + "px", width:(100 - percentage) + "%"}, trow);
					cellTblTask.style.lineHeight = "1px";
					imageProgressFill = dojo.create("div", {className:"ganttImageTaskProgressBg"}, cellTblTask);
					dojo.style(imageProgressFill, {width:((100 - percentage) * this.taskItem.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
				}
			}
		} else {
			if (percentage == 0) {
	...
	dojo.declare("dojox.gantt.GanttTaskItem", null, {constructor:function (configuration) {
