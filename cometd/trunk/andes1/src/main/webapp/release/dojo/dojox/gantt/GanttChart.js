...
if (!dojo._hasResource["dojox.gantt.GanttChart"]) {
	...
	(function () {
		dojo.declare("dojox.gantt.GanttChart", null, {constructor:function (configuration, node) {
			...
			var project = new dojox.gantt.GanttProjectItem({id:id, name:name, startDate:startDate});
			...
			dojo.xhrGet({url:_this.dataFilePath, sync:true, load:function (text, ioArgs) {
			...
			dojo.forEach(items, function (pItem) {
				...
				var project = new dojox.gantt.GanttProjectItem({id:pItem.id, name:pItem.name, startDate:new Date(startDate[0], (parseInt(startDate[1]) - 1), startDate[2])});
				var tItems = pItem.tasks;
				dojo.forEach(tItems, function (tItem) {
					var id = tItem.id, name = tItem.name, starttime = tItem.starttime.split("-");
					duration = tItem.duration, percentage = tItem.percentage, previousTaskId = tItem.previousTaskId, taskOwner = tItem.taskOwner;
					var task = new dojox.gantt.GanttTaskItem({id:id, name:name, startTime:new Date(starttime[0], (parseInt(starttime[1]) - 1), starttime[2]), duration:duration, percentage:percentage, previousTaskId:previousTaskId, taskOwner:taskOwner});
			...
			try {
				var td = dojo.xhrPost({url:_this.saveProgramPath, content:{filename:_this.dataFilePath, data:dojo.toJson(_this.getJSONData())}, handle:function (res, ioArgs) {
					if ((dojo._isDocumentOk(ioArgs.xhr)) || (ioArgs.xhr.status == 405)) {
			...
			var panelTask = dojo.create("div", {className:"ganttTaskPanel"});
			dojo.style(panelTask, {height:(this.contentHeight - this.panelTimeHeight - this.scrollBarWidth) + "px"});
			return panelTask;
			...
			var tblHeader = dojo.create("table", {cellPadding:"0px", border:"0px", cellSpacing:"0px", bgColor:"#FFFFFF", className:"ganttToolbar"}, panelHeader);
			...
			var zoomIn = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarZoomIn"}, firstRow);
			...
			var zoomOut = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarZoomOut"}, firstRow);
			...
			var micro = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarMicro"}, secondRow);
			...
			var tele = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarTele"}, secondRow);
			...
			var save = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarSave"}, thirdRow);
			...
			var load = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarLoad"}, thirdRow);
			...
			var panelNameTask = dojo.create("div", {innerHTML:"&nbsp;", className:"ganttPanelNames"});
			dojo.style(panelNameTask, {height:(this.contentHeight - this.panelTimeHeight - this.scrollBarWidth) + "px", width:this.maxWidthPanelNames + "px"});
			return panelNameTask;
		}, createPanelTime:function () {
			var panelTime = dojo.create("div", {className:"ganttPanelTime"});
			var tblTime = dojo.create("table", {cellPadding:"0px", border:"0px", cellSpacing:"0px", bgColor:"#FFFFFF", className:"ganttTblTime"}, panelTime);
			...
			var data = "Year   " + year;
			var newCell = dojo.create("td", {colSpan:count, align:"center", vAlign:"middle", className:"ganttYearNumber", innerHTML:this.pixelsPerDay * count > 20 ? data : "", innerHTMLData:data}, row);
			...
			var newCell = dojo.create("td", {colSpan:count, align:"center", vAlign:"middle", className:"ganttMonthNumber", innerHTML:this.pixelsPerDay * count > 30 ? data : "", innerHTMLData:data}, row);
			dojo.style(newCell, "width", (this.pixelsPerDay * count) + "px");
		}, addWeekInPanelTime:function (row, count, week) {
			var data = "Week   " + week;
			var newCell = dojo.create("td", {colSpan:count, align:"center", vAlign:"middle", className:"ganttWeekNumber", innerHTML:this.pixelsPerDay * count > 20 ? data : "", innerHTMLData:data}, row);
			dojo.style(newCell, "width", (this.pixelsPerDay * count) + "px");
		}, addDayInPanelTime:function (row) {
			var date = new Date(this.startDate);
			date.setDate(date.getDate() + parseInt(row.cells.length));
			var newCell = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttDayNumber", innerHTML:this.pixelsPerDay > 20 ? date.getDate() : "", innerHTMLData:String(date.getDate()), data:row.cells.length}, row);
			dojo.style(newCell, "width", this.pixelsPerDay + "px");
			(date.getDay() >= 5) && dojo.addClass(newCell, "ganttDayNumberWeekend");
			this._events.push(dojo.connect(newCell, "onmouseover", this, function (event) {
			...
			this._events.push(dojo.connect(newCell, "onmouseout", this, function (event) {
				var dayTime = event.target || event.srcElement;
				dayTime && dijit.hideTooltip(dayTime);
			}));
		}, addHourInPanelTime:function (row) {
			var newCell = dojo.create("td", {align:"center", vAlign:"middle", className:"ganttHourNumber", data:row.cells.length}, row);
			dojo.style(newCell, "width", this.pixelsPerDay + "px");
			var hourTable = dojo.create("table", {cellPadding:"0", cellSpacing:"0"}, newCell);
			var newRow = hourTable.insertRow(hourTable.rows.length);
			for (var i = 0; i < this.hsPerDay; i++) {
				var hourTD = dojo.create("td", {className:"ganttHourClass"}, newRow);
			...
			this.contentData.appendChild(this.createPanelTasks());
			var newCellTblControl = dojo.create("td", {vAlign:"top"});
			...
			newCellTblControl = dojo.create("td", {vAlign:"top"});
			...
			if (!this.panelNameHeadersCover) {
				...
				dojo.style(this.panelNameHeadersCover, {left:cpos.l + "px", top:cpos.t + "px", height:cpos.h + "px", width:cpos.w + "px", display:"none"});
			}
		}, postBindEvents:function () {
			var pos = dojo.position(this.tableControl, true);
			!dojo.isIE && this._events.push(dojo.connect(this.tableControl, "onmousemove", this, function (event) {
				...
				if (elem == this.panelNames.firstChild || elem == this.contentData.firstChild) {
					...
					if (hlTop != this.oldHLTop && hlTop < (pos.h - 50)) {
						if (this.highLightDiv) {
							...
							dojo.style(this.highLightDiv, {top:(pos.y + hlTop) + "px", left:pos.x + "px", width:(pos.w - 20) + "px", height:rowHeight + "px"});
						}
					}
					this.oldHLTop = hlTop;
				}
