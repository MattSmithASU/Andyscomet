...
if (!dojo._hasResource["dojox.gfx.canvas"]) {
	...
	(function () {
		var d = dojo, g = dojox.gfx, gs = g.shape, ga = g.arc, canvas = g.canvas, m = g.matrix, mp = m.multiplyPoint, pi = Math.PI, twoPI = 2 * pi, halfPI = pi / 2, pattrnbuffer = null;
		d.declare("dojox.gfx.canvas.Shape", gs.Shape, {_render:function (ctx) {
			ctx.save();
		...
		dojo.declare("dojox.gfx.canvas.Group", canvas.Shape, {constructor:function () {
		...
		dojo.declare("dojox.gfx.canvas.Rect", [canvas.Shape, gs.Rect], {_renderShape:function (ctx) {
		...
		dojo.declare("dojox.gfx.canvas.Ellipse", [canvas.Shape, gs.Ellipse], {setShape:function () {
		...
		dojo.declare("dojox.gfx.canvas.Circle", [canvas.Shape, gs.Circle], {_renderShape:function (ctx) {
			var s = this.shape;
			ctx.beginPath();
			ctx.arc(s.cx, s.cy, s.r, 0, twoPI, 1);
		}});
		dojo.declare("dojox.gfx.canvas.Line", [canvas.Shape, gs.Line], {_renderShape:function (ctx) {
			var s = this.shape;
			ctx.beginPath();
			ctx.moveTo(s.x1, s.y1);
			ctx.lineTo(s.x2, s.y2);
		}});
		dojo.declare("dojox.gfx.canvas.Polyline", [canvas.Shape, gs.Polyline], {setShape:function () {
			this.inherited(arguments);
		...
		dojo.declare("dojox.gfx.canvas.Image", [canvas.Shape, gs.Image], {setShape:function () {
			this.inherited(arguments);
			var img = new Image();
		...
		dojo.declare("dojox.gfx.canvas.Text", [canvas.Shape, gs.Text], {_setFont:function () {
			if (this.fontStyle) {
		...
		dojo.declare("dojox.gfx.canvas.Path", [canvas.Shape, g.path.Path], {constructor:function () {
			this.lastControl = {};
		}, setShape:function () {
			this.canvasPath = [];
		...
		dojo.declare("dojox.gfx.canvas.TextPath", [canvas.Shape, g.path.TextPath], {_renderShape:function (ctx) {
			var s = this.shape;
		}, _setText:function () {
		}, _setFont:function () {
		}});
		...
		canvas.createSurface = function (parentNode, width, height) {
			...
			var s = new canvas.Surface(), p = d.byId(parentNode), c = p.ownerDocument.createElement("canvas");
