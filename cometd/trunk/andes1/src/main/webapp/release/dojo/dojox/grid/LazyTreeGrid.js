...
if (!dojo._hasResource["dojox.grid.LazyTreeGrid"]) {
	...
	dojo.declare("dojox.grid._TreeGridContentBuilder", dojox.grid._ContentBuilder, {generateHtml:function (inDataIndex, inRowIndex) {
		var html = this.getTableArray(), grid = this.grid, v = this.view, cells = v.structure.cells, item = grid.getItem(inRowIndex), level = 0, treePath = grid.cache.getTreePathByRowIndex(inRowIndex), rowStack = [], toggleClasses = [];
	...
	dojox.grid.cells.LazyTreeCell = dojo.mixin(dojo.clone(dojox.grid.cells.TreeCell), {formatAtLevel:function (inRowIndexes, inItem, level, summaryRow, toggleClass, cellClasses, inRowIndex) {
		if (!inItem) {
		...
		if (this.isCollapsable) {
			...
			ret = "<span " + dojo._scopeName + "Type=\"dojox.grid._LazyExpando\" level=\"" + level + "\" class=\"dojoxGridExpando\"" + "\" toggleClass=\"" + toggleClass + "\" itemId=\"" + id + "\" cellIdx=\"" + this.index + "\"></span>";
		}
		...
		var info = this.grid.edit.info, d = this.get ? this.get(inRowIndexes[0], inItem, inRowIndexes) : (this.value || this.defaultValue);
	...
	dojo.declare("dojox.grid._LazyTreeLayout", dojox.grid._Layout, {setStructure:function (inStructure) {
		var s = inStructure;
		var g = this.grid;
	...
	dojo.declare("dojox.grid.TreeGridItemCache", null, {unInit:true, items:null, constructor:function (grid) {
	...
	}, cacheItem:function (rowIdx, cacheObj) {
		this.items[rowIdx] = dojo.mixin({item:null, treePath:"", expandoStatus:false}, cacheObj);
	}, insertItem:function (rowIdx, cacheObj) {
		this.items.splice(rowIdx, 0, dojo.mixin({item:null, treePath:"", expandoStatus:false}, cacheObj));
		...
		this.unInit = false;
	}, getItemByRowIndex:function (rowIdx) {
		...
		return null;
	}, getTreePathByRowIndex:function (rowIdx) {
		return this.items[rowIdx] ? this.items[rowIdx].treePath : null;
	}, getExpandoStatusByRowIndex:function (rowIdx) {
		...
		return null;
	}, updateCache:function (rowIdx, cacheObj) {
	...
	dojo.declare("dojox.grid.LazyTreeGrid", dojox.grid.TreeGrid, {treeModel:null, _layoutClass:dojox.grid._LazyTreeLayout, colSpans:null, postCreate:function () {
		...
		if (fetchedItems.length === count) {
			...
			var level = "", nextRowLevel = "", startRowIdx = start, startTreePath = this.cache.getTreePathByRowIndex(start);
			count = 0;
			for (i = start + 1; i < start + this.rowsPerPage; i++) {
				...
				if (level !== nextRowLevel) {
					this.reqQueue.push({startTreePath:startTreePath, startRowIdx:startRowIdx, count:count + 1});
					count = 0;
					startRowIdx = i;
		...
		if (level === 0) {
			this.store.fetch({start:parseInt(this.reqQueue[idx].startTreePath, 10), startRowIdx:this.reqQueue[idx].startRowIdx, count:this.reqQueue[idx].count, query:this.query, sort:this.getSortProps(), queryOptions:this.queryOptions, onBegin:onBegin, onComplete:onComplete, onError:onError});
		} else {
			...
			this.queryObj = {start:parseInt(startIdx, 10), startRowIdx:this.reqQueue[idx].startRowIdx, count:this.reqQueue[idx].count, parentId:parentId, sort:this.getSortProps()};
		...
		for (var i = 0; i < count; i++) {
			...
			if (treePath) {
				if (!this.cache.getItemByRowIndex(startRowIdx + i)) {
					this.cache.cacheItem(startRowIdx + i, {item:items[start + i], treePath:treePath, expandoStatus:false});
				}
			}
		}
		...
		if (open) {
			...
			var queryObj = {start:0, count:this.keepRows, parentId:parentId, sort:this.getSortProps()};
		...
		for (i = this.expandoRowIndex + 1; j < size; i++, j++) {
			this.cache.insertItem(i, {item:null, treePath:parentTreePath + "/" + j, expandoStatus:false});
		}
		this.updateRowCount(this.cache.items.length);
		...
		var dk = dojo.keys, target = e.target, expando = target && target.firstChild ? dijit.byId(target.firstChild.id) : null;
