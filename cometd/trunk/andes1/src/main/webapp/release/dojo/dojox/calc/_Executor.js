...
if (!dojo._hasResource["dojox.calc._Executor"]) {
	...
	(function () {
		...
		dojo.declare("dojox.calc._Executor", [dijit._Widget, dijit._Templated], {templateString:"<iframe src=\"" + dojo.moduleUrl("dojox.calc", "_ExecutorIframe.html") + "\" style=\"display:none;\" onload=\"if(arguments[0] && arguments[0].Function)" + dijit._scopeName + ".byNode(this)._onLoad(arguments[0])\"></iframe>", _onLoad:function (env) {
			calcEnv = env;
			env.outerPrompt = window.prompt;
			env.dojox = {math:{}};
			for (var f in dojox.math) {
				env.dojox.math[f] = dojo.hitch(dojox.math, f);
			}
