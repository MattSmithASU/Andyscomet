...
if (!dojo._hasResource["dojox.drawing.tools.custom.Axes"]) {
	...
	dojox.drawing.tools.custom.Axes = dojox.drawing.util.oo.declare(dojox.drawing.stencil.Path, function (options) {
		...
		if (this.style.zAxisEnabled) {
			...
			dojo.mixin(ops, {container:this.container.createGroup(), style:this.style, showAngle:false, label:null});
		...
		if (this.style.zAxisEnabled) {
			this.labelZ = new dojox.drawing.annotations.Label(dojo.mixin(props, {labelPosition:this.setLabelZ}));
		}
	}, setLabelX:function () {
