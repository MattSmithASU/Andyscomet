			...
			if (range) {
				...
				if (quoteIt) {
					...
					if (range.startContainer === range.endContainer) {
						...
						curNode = start;
						while (curNode.nextSibling && dojo.withGlobal(ed.window, "inSelection", dijit._editor.selection, [curNode])) {
					...
					if (range.startContainer === range.endContainer) {
						...
						if (found) {
							...
							if (lastChild) {
								dojo.withGlobal(ed.window, "selectElementChildren", dijit._editor.selection, [lastChild]);
								dojo.withGlobal(ed.window, "collapse", dijit._editor.selection, [true]);
							}
						}
					} else {
						...
						var selectedNodes = [];
						var cNode = start;
						while (cNode && cNode.nextSibling && dojo.withGlobal(ed.window, "inSelection", dijit._editor.selection, [cNode])) {
