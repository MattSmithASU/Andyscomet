...
if (!dojo._hasResource["dojox.mobile.SpinWheel"]) {
	dojo._hasResource["dojox.mobile.SpinWheel"] = true;
	...
	dojo.declare("dojox.mobile.SpinWheel", [dijit._WidgetBase, dijit._Container, dijit._Contained], {slotClasses:[], slotProps:[], centerPos:0, buildRendering:function () {
		...
		for (var i = 0; i < this.slotClasses.length; i++) {
			this.slots.push(dojo.getObject(this.slotClasses[i])(this.slotProps[i]));
	...
	dojo.declare("dojox.mobile.SpinWheelSlot", [dijit._WidgetBase, dijit._Contained, dojox.mobile._ScrollableMixin], {items:[], labels:[], labelFrom:0, labelTo:0, maxSpeed:200, minItems:100, centerPos:0, value:"", scrollBar:false, constraint:false, buildRendering:function () {
		this.inherited(arguments);
		...
		this.containerNode.style.height = (dojo.global.innerHeight || dojo.doc.documentElement.clientHeight) * 2 + "px";
		this.panelNodes = [];
		for (k = 0; k < 3; k++) {
			...
			for (j = 0; j < n; j++) {
				for (i = 0; i < len; i++) {
					dojo.create("DIV", {className:"mblSpinWheelSlotLabel", name:this.items[i][0], innerHTML:this._cv(this.items[i][1])}, this.panelNodes[k]);
				}
			}
