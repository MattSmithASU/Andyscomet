...
if (!dojo._hasResource["dojox.date.timezone"]) {
	...
	(function (_d) {
		...
		function loadZoneFile(fileName) {
			...
			_d.xhrGet({url:timezoneFileBasePath + "/" + fileName, sync:true, handleAs:"olson-zoneinfo", load:loadZoneData, error:errorLoadingZoneFile});
		}
		...
		var oLocaleFmt = _d.date.locale.format, oGetZone = _d.date.locale._getZone;
