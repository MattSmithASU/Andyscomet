...
if (!dojo._hasResource["andes.error"]) {
	...
	(function () {
		...
		andes.error = function (spec) {
			var message = spec.message || "An unknown error occurred.", title = spec.title || "Error", dialogType = spec.dialogType || 0;
			dialog.set({content:message, title:title, dialogType:dialogType});
			dialog.show();
		};
		andes.error.FATAL = 0;
		andes.error.OK = 1;
		dojo.declare("andes.error._Error", dijit.Dialog, {postCreate:function () {
			...
			var btn_OK = new dijit.form.Button({label:"OK", type:"submit"}, dojo.create("div", null, page1));
		...
		dojo.addOnLoad(function () {
			dialog = new andes.error._Error({id:"andesErrorDialog", title:"Error", style:"width:400px"});
