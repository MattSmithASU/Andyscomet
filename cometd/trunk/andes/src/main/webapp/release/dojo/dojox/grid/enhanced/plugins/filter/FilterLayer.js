...
if (!dojo._hasResource["dojox.grid.enhanced.plugins.filter.FilterLayer"]) {
	...
	(function () {
		var ns = dojox.grid.enhanced.plugins, cmdSetFilter = "filter", cmdClearFilter = "clear", hitchIfCan = function (scope, func) {
		...
		dojo.declare("dojox.grid.enhanced.plugins.filter.ClientSideFilterLayer", [ns._StoreLayer, ns.filter._FilterLayerMixin], {_storeSize:-1, _fetchAll:true, constructor:function (args) {
			...
			try {
				...
				if (!filterRequest) {
					...
					var sortStr;
					if (dojo.isArray(userRequest.sort) && userRequest.sort.length > 0 && (sortStr = dojo.toJson(userRequest.sort)) != this._lastSortInfo) {
						this.invalidate();
			...
			if (this._filter) {
				var m = dojo.clone(mapping), alreadyUpdated = {};
				for (var r in m) {
					r = parseInt(r, 10);
