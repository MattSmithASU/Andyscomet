...
if (!dojo._hasResource["dojo.data.ItemFileReadStore"]) {
	...
	dojo.declare("dojo.data.ItemFileReadStore", null, {constructor:function (keywordParameters) {
		...
		var self = this, filter = function (requestArgs, arrayOfItems) {
			var items = [], i, key;
		...
		if (this._queuedFetches.length > 0) {
			for (var i = 0; i < this._queuedFetches.length; i++) {
				var fData = this._queuedFetches[i], delayedQuery = fData.args, delayedFilter = fData.filter;
				if (delayedFilter) {
		...
		var item = null;
		if (this._itemsByIdentity && Object.hasOwnProperty.call(this._itemsByIdentity, identity)) {
