...
if (!dojo._hasResource["dojox.gfx.gradient"]) {
	...
	(function () {
		...
		dojox.gfx.gradient.rescale = function (stops, from, to) {
			...
			if (to <= stops[0].offset) {
				...
				if (from >= stops[len - 1].offset) {
					...
					if (i) {
						prev = stops[i - 1];
						newStops.push({offset:0, color:d.blendColors(new C(prev.color), new C(stop.color), (from - prev.offset) / (stop.offset - prev.offset))});
					} else {
					...
					if (i < len) {
						prev = stops[i - 1];
						newStops.push({offset:1, color:d.blendColors(new C(prev.color), new C(stop.color), (to - prev.offset) / (stop.offset - prev.offset))});
					} else {
		...
		dojox.gfx.gradient.project = function (matrix, grad, tl, rb, ttl, trb) {
			...
			var f1 = m.multiplyPoint(matrix, grad.x1, grad.y1), f2 = m.multiplyPoint(matrix, grad.x2, grad.y2), angle = Math.atan2(f2.y - f1.y, f2.x - f1.x), project = m.project(f2.x - f1.x, f2.y - f1.y), pf1 = m.multiplyPoint(project, f1), pf2 = m.multiplyPoint(project, f2), shiftAndRotate = new m.Matrix2D([m.rotate(-angle), {dx:-pf1.x, dy:-pf1.y}]), scale = m.multiplyPoint(shiftAndRotate, pf2).x, points = [getPoint(tl.x, tl.y, matrix, project, shiftAndRotate, scale), getPoint(rb.x, rb.y, matrix, project, shiftAndRotate, scale), getPoint(tl.x, rb.y, matrix, project, shiftAndRotate, scale), getPoint(rb.x, tl.y, matrix, project, shiftAndRotate, scale)].sort(sortPoints), from = points[0].o, to = points[3].o, stops = dojox.gfx.gradient.rescale(grad.colors, from, to), angle2 = Math.atan2(points[3].r.y - points[0].r.y, points[3].r.x - points[0].r.x);
