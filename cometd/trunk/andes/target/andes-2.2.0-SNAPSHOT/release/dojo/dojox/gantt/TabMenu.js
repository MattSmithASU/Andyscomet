...
if (!dojo._hasResource["dojox.gantt.TabMenu"]) {
	...
	(function () {
		dojo.declare("dojox.gantt.TabMenu", null, {constructor:function (chart) {
			...
			var cell = dojo.create("td", {className:"ganttContextMenuItem", innerHTML:tab.Description});
			dojo.attr(cell, "tabIndex", 0);
			this.ganttChart._events.push(dojo.connect(cell, "onclick", this, function () {
				try {
					this.hide();
					tab.show();
				}
			...
			this.ganttChart._events.push(dojo.connect(cell, "onkeydown", this, function (e) {
			...
			this.ganttChart._events.push(dojo.connect(cell, "onmouseover", this, function () {
				dojo.addClass(cell, "ganttContextMenuItemHover");
			}));
			this.ganttChart._events.push(dojo.connect(cell, "onmouseout", this, function () {
			...
			if (object.constructor == dojox.gantt.GanttTaskControl) {
				...
				if (object.constructor == dojox.gantt.GanttProjectControl) {
					dojo.forEach(this.arrTabs, function (tab) {
						if (tab.type == "p") {
							tab.object = object;
							this.addItemMenuPanel(tab);
			...
			var menuBox = dojo.position(this.menuPanel, true), bBox = dojo.position(this.ganttChart.content, true), pos = dojo.coords(elem, true);
		...
		dojo.declare("dojox.gantt.contextMenuTab", null, {constructor:function (id, description, type, showOInfo, tabMenu, withDefaultValue) {
			this.id = id;
			this.arrItems = [];
			...
			var content = this.tabMenu.paneContentArea.firstChild.rows[1].cells[0].firstChild, action = this.tabMenu.paneActionBar;
