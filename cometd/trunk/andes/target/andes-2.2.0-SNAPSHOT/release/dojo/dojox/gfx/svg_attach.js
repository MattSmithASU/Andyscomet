	...
	(function () {
		...
		function attachTransform(object) {
			...
			if (matrix.match(/^matrix\(.+\)$/)) {
				...
				object.matrix = g.matrix.normalize({xx:parseFloat(t[0]), xy:parseFloat(t[2]), yx:parseFloat(t[1]), yy:parseFloat(t[3]), dx:parseFloat(t[4]), dy:parseFloat(t[5])});
			} else {
				object.matrix = null;
			}
		}
		function attachFont(object) {
			var fontStyle = object.fontStyle = dojo.clone(g.defaultFont), r = object.rawNode;
		...
		function attachText(object) {
			var shape = object.shape = dojo.clone(g.defaultText), r = object.rawNode;
		...
		function attachTextPath(object) {
			var shape = object.shape = dojo.clone(g.defaultTextPath), r = object.rawNode;
			shape.align = r.getAttribute("text-anchor");
			shape.decoration = r.getAttribute("text-decoration");
			shape.rotated = parseFloat(r.getAttribute("rotate")) != 0;
			shape.kerning = r.getAttribute("kerning") == "auto";
