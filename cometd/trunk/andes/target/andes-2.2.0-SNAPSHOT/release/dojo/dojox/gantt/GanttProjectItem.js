...
if (!dojo._hasResource["dojox.gantt.GanttProjectItem"]) {
	...
	dojo.declare("dojox.gantt.GanttProjectControl", null, {constructor:function (ganttChart, projectItem) {
		...
		dojo.style(projectItem, {"left":this.posX + "px", "width":this.duration * this.ganttChart.pixelsPerWorkHour + "px"});
		...
		if (this.percentage != -1) {
			if (this.percentage != 0) {
				...
				dojo.style(imageProgress, {width:(!this.duration ? 1 : (this.percentage * this.duration * this.ganttChart.pixelsPerWorkHour / 100)) + "px", height:this.ganttChart.heightTaskItem + "px"});
			}
			if (this.percentage != 100) {
				var cellprojectItem = rowprojectItem.lastChild;
				cellprojectItem.width = (100 - this.percentage) + "%";
				var imageProgress = cellprojectItem.firstChild;
				dojo.style(imageProgress, {width:(!this.duration ? 1 : ((100 - this.percentage) * this.duration * this.ganttChart.pixelsPerWorkHour / 100)) + "px", height:this.ganttChart.heightTaskItem + "px"});
			}
		} else {
			var cellprojectItem = rowprojectItem.firstChild;
			cellprojectItem.width = "1px";
			var imageProgress = cellprojectItem.firstChild;
			dojo.style(imageProgress, {width:"1px", height:this.ganttChart.heightTaskItem + "px"});
		}
		var divTaskInfo = projectItem.lastChild;
		var tblTaskInfo = divTaskInfo.firstChild;
		dojo.style(tblTaskInfo, {height:this.ganttChart.heightTaskItem + "px", width:(!this.duration ? 1 : (this.duration * this.ganttChart.pixelsPerWorkHour)) + "px"});
		...
		var posX = (this.posX + this.duration * this.ganttChart.pixelsPerWorkHour + 10);
		var divDesc = dojo.create("div", {innerHTML:this.getDescStr(), className:"ganttDescProject"});
		...
		var projectItem = dojo.create("div", {id:this.project.id, className:"ganttProjectItem"});
		dojo.style(projectItem, {left:this.posX + "px", top:this.posY + "px", width:this.duration * this.ganttChart.pixelsPerWorkHour + "px"});
		var tblProjectItem = dojo.create("table", {cellPadding:"0", cellSpacing:"0", className:"ganttTblProjectItem"}, projectItem);
		var width = this.duration * this.ganttChart.pixelsPerWorkHour;
		tblProjectItem.width = ((width == 0) ? 1 : width) + "px";
		tblProjectItem.style.width = ((width == 0) ? 1 : width) + "px";
		...
		if (this.percentage != -1) {
			if (this.percentage != 0) {
				var cellprojectItem = dojo.create("td", {width:this.percentage + "%"}, rowprojectItem);
				cellprojectItem.style.lineHeight = "1px";
				var imageProgress = dojo.create("div", {className:"ganttImageProgressFilled"}, cellprojectItem);
				dojo.style(imageProgress, {width:(this.percentage * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
			}
			if (this.percentage != 100) {
				var cellprojectItem = dojo.create("td", {width:(100 - this.percentage) + "%"}, rowprojectItem);
				cellprojectItem.style.lineHeight = "1px";
				var imageProgress = dojo.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
				dojo.style(imageProgress, {width:((100 - this.percentage) * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
			}
		} else {
			var cellprojectItem = dojo.create("td", {width:"1px"}, rowprojectItem);
			cellprojectItem.style.lineHeight = "1px";
			var imageProgress = dojo.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
			dojo.style(imageProgress, {width:"1px", height:this.ganttChart.heightTaskItem + "px"});
		}
		var divTaskInfo = dojo.create("div", {className:"ganttDivTaskInfo"});
		var tblTaskInfo = dojo.create("table", {cellPadding:"0", cellSpacing:"0", height:this.ganttChart.heightTaskItem + "px", width:((this.duration * this.ganttChart.pixelsPerWorkHour == 0) ? 1 : this.duration * this.ganttChart.pixelsPerWorkHour) + "px"}, divTaskInfo);
		...
		var divName = dojo.create("div", {className:"ganttProjectNameItem", innerHTML:this.project.name, title:this.project.name});
		dojo.style(divName, {left:"5px", top:this.posY + "px"});
		...
		if ((percentage > 0) && (percentage < 100) && (this.percentage > 0) && (this.percentage < 100)) {
			...
			if (((percentage == 0) || (percentage == 100)) && (this.percentage > 0) && (this.percentage < 100)) {
				...
				if (((percentage == 0) || (percentage == 100)) && ((this.percentage == 0) || (this.percentage == 100))) {
					...
					if (((percentage > 0) || (percentage < 100)) && ((this.percentage == 0) || (this.percentage == 100))) {
						rc0.parentNode.removeChild(rc0);
						var cellprojectItem = dojo.create("td", {width:percentage + "%"}, prow);
						cellprojectItem.style.lineHeight = "1px";
						var imageProgress = dojo.create("div", {className:"ganttImageProgressFilled"}, cellprojectItem);
						dojo.style(imageProgress, {width:(percentage * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
						cellprojectItem = dojo.create("td", {width:(100 - percentage) + "%"}, prow);
						cellprojectItem.style.lineHeight = "1px";
						imageProgress = dojo.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
						dojo.style(imageProgress, {width:((100 - percentage) * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
					} else {
						if (this.percentage == -1) {
							if (percentage == 100) {
								dojo.removeClass(rc0.firstChild, "ganttImageProgressBg");
								...
								if (percentage < 100 && percentage > 0) {
									...
									var cellprojectItem = dojo.create("td", {width:percentage + "%"}, prow);
									cellprojectItem.style.lineHeight = "1px";
									imageProgress = dojo.create("div", {className:"ganttImageProgressFilled"}, cellprojectItem);
									dojo.style(imageProgress, {width:(percentage * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
									cellprojectItem = dojo.create("td", {width:(100 - percentage) + "%"}, prow);
									cellprojectItem.style.lineHeight = "1px";
									imageProgress = dojo.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
		...
		if (task) {
			var tItem0 = task.cTaskItem[0], tNameItem0 = task.cTaskNameItem[0], tItem1 = task.cTaskItem[1], tNameItem1 = task.cTaskNameItem[1], tItem2 = task.cTaskItem[2], tNameItem2 = task.cTaskNameItem[2];
		...
		if ((parentTaskId) && (parentTaskId != "")) {
			...
			task = new dojox.gantt.GanttTaskItem({id:id, name:name, startTime:startTime, duration:duration, percentage:percentage, previousTaskId:previousTaskId, taskOwner:taskOwner});
		...
		var tNameItem0 = task.cTaskNameItem[0], tNameItem1 = task.cTaskNameItem[1], tNameItem2 = task.cTaskNameItem[2], tItem0 = task.cTaskItem[0], tItem1 = task.cTaskItem[1], tItem2 = task.cTaskItem[2];
	...
	dojo.declare("dojox.gantt.GanttProjectItem", null, {constructor:function (configuration) {
