...
(function () {
	...
	var attachFill = function (object) {
		...
		if (fo.on && fo.type == "gradient") {
			var fillStyle = dojo.clone(g.defaultLinearGradient), rad = m._degToRad(fo.angle);
			...
			if (fo.on && fo.type == "gradientradial") {
				var fillStyle = dojo.clone(g.defaultRadialGradient), w = parseFloat(r.style.width), h = parseFloat(r.style.height);
	...
	var attachStroke = function (object) {
		...
		var strokeStyle = object.strokeStyle = dojo.clone(g.defaultStroke), rs = r.stroke;
	...
	var attachLine = function (object) {
		var shape = object.shape = dojo.clone(g.defaultLine), p = object.rawNode.path.v.match(g.pathVmlRegExp);
		do {
	...
	var attachPolyline = function (object) {
		var shape = object.shape = dojo.clone(g.defaultPolyline), p = object.rawNode.path.v.match(g.pathVmlRegExp);
		do {
			if (p.length < 3 || p[0] != "m") {
				break;
			}
	...
	var attachText = function (object) {
		var shape = object.shape = dojo.clone(g.defaultText), r = object.rawNode, p = r.path.v.match(g.pathVmlRegExp);
		do {
			if (!p || p.length != 7) {
				break;
			}
	...
	var attachFont = function (object) {
		var fontStyle = object.fontStyle = dojo.clone(g.defaultFont), c = object.rawNode.childNodes, i = 0;
		for (; i < c.length && c[i].tagName == "textpath"; ++i) {
		}
		if (i >= c.length) {
	...
	var attachPath = function (object) {
		var shape = object.shape = dojo.clone(g.defaultPath), p = object.rawNode.path.v.match(g.pathVmlRegExp), t = [], skip = false, map = g.Path._pathVmlToSvgMap;
