	...
	(function () {
		...
		dojo.declare("dojox.grid.enhanced.plugins.Selector", dojox.grid.enhanced._Plugin, {name:"selector", constructor:function (grid, args) {
			...
			switch (type) {
			  ...
			  case "col":
			  case "row":
				return dojo.map(includeExceptions ? this._selected[type] : dojo.filter(this._selected[type], function (item) {
			...
			switch (type) {
			  ...
			  case "col":
			  case "row":
				return (includeExceptions ? this._selected[type] : dojo.filter(this._selected[type], function (item) {
					return item.except.length === 0;
				})).length;
			}
			return 0;
			...
			var g = this.grid, _this = this, dp = dojo.partial, starter = function (type, e) {
				if (type === "row") {
			...
			var g = this.grid, f = g.focus, _this = this, dk = dojo.keys, keyboardSelectReady = 1, duringKeyboardSelect = 2, onmove = function (type, createNewEnd, rowStep, colStep, evt) {
			...
			f.addArea({name:"cellselect", onMove:dojo.partial(onmove, "cell", function (type, rowStep, colStep, evt) {
			...
			f.addArea({name:"colselect", onMove:dojo.partial(onmove, "col", function (type, rowStep, colStep, evt) {
				var current = _this._currentPoint[type];
			...
			f.addArea({name:"rowselect", onMove:dojo.partial(onmove, "row", function (type, rowStep, colStep, evt) {
			...
			if (type == "cell") {
				...
				items = dojo.filter(items, function (item) {
					return dojo.indexOf(colMakedup, item) < 0 && dojo.indexOf(rowMakedup, item) < 0 && !cells[item.col].hidden && !cells[item.col].notselectable;
				});
			} else {
				if (type == "col") {
					items = dojo.filter(items, function (item) {
			...
			try {
				...
				switch (type) {
				  case "col":
					return index >= 0 && index < g.layout.cells.length && dojo.isArray(item.except) && (allowNotSelectable || !g.layout.cells[index].notselectable);
				  case "row":
