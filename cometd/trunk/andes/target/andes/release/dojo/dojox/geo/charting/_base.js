...
if (!dojo._hasResource["dojox.geo.charting._base"]) {
	...
	(function () {
		...
		dgc.showTooltip = function (innerHTML, gfxObject, position) {
		...
		dgc._normalizeArround = function (gfxObject) {
			...
			gfxObject.x = dojo.coords(gfxDomContainer, true).x + point.x, gfxObject.y = dojo.coords(gfxDomContainer, true).y + point.y, gfxObject.width = bbox.width * realMatrix.xx, gfxObject.height = bbox.height * realMatrix.yy;
			return gfxObject;
		};
