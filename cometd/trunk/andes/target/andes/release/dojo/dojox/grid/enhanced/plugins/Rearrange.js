	...
	dojo.declare("dojox.grid.enhanced.plugins.Rearrange", dojox.grid.enhanced._Plugin, {name:"rearrange", constructor:function (grid, args) {
		...
		for (i = colsToMove[0]; i <= maxCol; ++i) {
			...
			if (j >= 0) {
				...
				if (i < targetPos && leftCount > 0) {
					mapping[i] = i - leftCount;
				} else {
					if (i >= targetPos && rightCount > 0) {
		...
		if (s.getFeatures()["dojo.data.api.Write"]) {
			...
			s.save({onComplete:function () {
				dojo.publish("dojox/grid/rearrange/move/" + g.id, ["cell", {"from":cellsToMove, "to":target}]);
			}});
		}
		...
		if (s.getFeatures()["dojo.data.api.Write"]) {
			...
			s.save({onComplete:function () {
				setTimeout(function () {
					dojo.publish("dojox/grid/rearrange/copy/" + g.id, ["cell", {"from":cellsToMove, "to":target}]);
				}, 0);
			}});
		}
		...
		try {
			var g = this.grid, s = g.store, rowCnt = g.rowCount, mapping = {}, obj = {idx:0}, newRows = [], i, _this = this;
			...
			if (s.getFeatures()["dojo.data.api.Write"]) {
				if (sourceGrid) {
					var srcg = sourceGrid, srcs = srcg.store, thisItem;
					for (i = 0; !thisItem; ++i) {
					...
					if (rowsToMove.length && dojo.isObject(rowsToMove[0])) {
