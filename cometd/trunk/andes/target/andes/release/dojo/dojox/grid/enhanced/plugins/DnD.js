...
if (!dojo._hasResource["dojox.grid.enhanced.plugins.DnD"]) {
	...
	(function () {
		...
		dojo.declare("dojox.grid.enhanced.plugins.DnD", dojox.grid.enhanced._Plugin, {name:"dnd", _targetAnchorBorderWidth:2, _copyOnly:false, _config:{"row":{"within":true, "in":true, "out":true}, "col":{"within":true, "in":true, "out":true}, "cell":{"within":true, "in":true, "out":true}}, constructor:function (grid, args) {
			...
			var i, headPos, left, target, ex = evt.clientX, cells = this.grid.layout.cells, ltr = dojo._isBodyLtr(), headers = this._getVisibleHeaders();
			...
			var cell = g.layout.cells[i], rowIndex = g.scroller.firstVisibleRow, nodePos = dojo.position(cell.getNode(rowIndex));
			...
			var s = this._dndRegion.selected[0], origin = this._dndRegion.handle, g = this.grid, ltr = dojo._isBodyLtr(), cells = g.layout.cells, headPos, minPos, maxPos, headers, height, width, left, top, minCol, maxCol, i, preSpan = origin.col - s.min.col, postSpan = s.max.col - origin.col, leftTopDiv, rightBottomDiv;
			if (!targetAnchor.childNodes.length) {
				leftTopDiv = dojo.create("div", {"class":"dojoxGridCellBorderLeftTopDIV"}, targetAnchor);
				rightBottomDiv = dojo.create("div", {"class":"dojoxGridCellBorderRightBottomDIV"}, targetAnchor);
			} else {
			...
			var cell = cells[i], rowIndex = g.scroller.firstVisibleRow, nodePos = dojo.position(cell.getNode(rowIndex));
			while (nodePos.y + nodePos.h < evt.clientY) {
				if (++rowIndex < g.rowCount) {
					nodePos = dojo.position(cell.getNode(rowIndex));
				} else {
			...
			dojo.style(leftTopDiv, {"width":(leftTopCellPos.w - anchorBorderSize) + "px", "height":(leftTopCellPos.h - anchorBorderSize) + "px"});
			var rightBottomCellPos = dojo.position(cells[maxCol].getNode(maxRow));
			dojo.style(rightBottomDiv, {"width":(rightBottomCellPos.w - anchorBorderSize) + "px", "height":(rightBottomCellPos.h - anchorBorderSize) + "px"});
			...
			try {
				...
				var height, width, left, top, targetAnchor = this._targetAnchor[t], pos = dojo.position(this._container);
				if (!targetAnchor) {
					targetAnchor = this._targetAnchor[t] = dojo.create("div", {"class":(t == "cell") ? "dojoxGridCellBorderDIV" : "dojoxGridBorderDIV"});
				...
				if (typeof height == "number" && typeof width == "number" && typeof left == "number" && typeof top == "number") {
					dojo.style(targetAnchor, {"height":height + "px", "width":width + "px", "left":left + "px", "top":top + "px"});
			...
			switch (type) {
			  case "cell":
				ranges = ranges[0];
				res = g.store.getFeatures()["dojo.data.api.Write"] && (ranges.max.row - ranges.min.row) <= rowCnt && dojo.filter(sourcePlugin.grid.layout.cells, function (cell) {
		...
		dojo.declare("dojox.grid.enhanced.plugins.GridDnDElement", null, {constructor:function (dndPlugin) {
		...
		dojo.declare("dojox.grid.enhanced.plugins.GridDnDSource", dojo.dnd.Source, {accept:["grid/cells", "grid/rows", "grid/cols"], constructor:function (node, param) {
		...
		dojo.declare("dojox.grid.enhanced.plugins.GridDnDAvatar", dojo.dnd.Avatar, {construct:function () {
			...
			var a = dojo.create("table", {"border":"0", "cellspacing":"0", "class":"dojoxGridDndAvatar", "style":{position:"absolute", zIndex:"1999", margin:"0px"}}), source = this.manager.source, b = dojo.create("tbody", null, a), tr = dojo.create("tr", null, b), td = dojo.create("td", {"class":"dojoxGridDnDIcon"}, tr);
			if (this.isA11y) {
			...
			dojo.create("span", {"class":"dojoxGridDnDItemCount", "innerHTML":source.generateText ? this._generateText() : ""}, td);
