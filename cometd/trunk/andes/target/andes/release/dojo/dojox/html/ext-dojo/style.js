...
if (!dojo._hasResource["dojox.html.ext-dojo.style"]) {
	...
	dojo.mixin(dojox.html["ext-dojo"].style, {supportsTransform:true, _toPx:function (measure) {
		...
		dojo.style = function (node, style, value) {
			var n = dojo.byId(node), tr = (style == "transform"), to = (style == "transformOrigin"), args = arguments.length;
			if (args == 3) {
				if (tr) {
		...
		var to = dojo.trim(transformOrigin).replace(" top", " 0").replace("left ", "0 ").replace(" center", "50%").replace("center ", "50% ").replace(" bottom", " 100%").replace("right ", "100% ").replace(/\s+/, " "), toAry = to.split(" "), n = dojo.byId(node), t = this.getTransform(n), validOrigin = true;
		...
		var t = transform.replace(/\s/g, ""), n = dojo.byId(node), transforms = t.split(")"), toRad = 1, toRad1 = 1, mstr = "DXImageTransform.Microsoft.Matrix", hasAttr = dojo.hasAttr, attr = dojo.attr, PI = Math.PI, cos = Math.cos, sin = Math.sin, tan = Math.tan, max = Math.max, min = Math.min, abs = Math.abs, degToRad = PI / 180, gradToRad = PI / 200, ct = "", currentTransform = "", matchingTransforms = [], x0 = 0, y0 = 0, dx = 0, dy = 0, xc = 0, yc = 0, a = 0, m11 = 1, m12 = 0, m21 = 0, m22 = 1, tx = 0, ty = 0, props = [m11, m12, m21, m22, tx, ty], hasMatrix = false, ds = dojo.style, newPosition = ds(n, "position") == "absolute" ? "absolute" : "relative", w = ds(n, "width") + ds(n, "paddingLeft") + ds(n, "paddingRight"), h = ds(n, "height") + ds(n, "paddingTop") + ds(n, "paddingBottom"), toPx = this._toPx;
		...
		try {
			var n = dojo.byId(node), item = n.filters.item(0);
			return "matrix(" + item.M11 + ", " + item.M12 + ", " + item.M21 + ", " + item.M22 + ", " + (dojo.attr(node, "dojo-transform-tx") || "0") + ", " + (dojo.attr(node, "dojo-transform-ty") || "0") + ")";
		}
		catch (e) {
