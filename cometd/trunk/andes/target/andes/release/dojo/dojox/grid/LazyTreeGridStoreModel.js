	...
	dojo.declare("dojox.grid.LazyTreeGridStoreModel", dijit.tree.ForestStoreModel, {serverStore:false, constructor:function (args) {
		...
		if (queryObj) {
			...
			if (parentItem === this.root) {
				this.root.size = 0;
				this.store.fetch({start:start, count:count, sort:sort, query:this.query, onBegin:dojo.hitch(this, function (size) {
					this.root.size = size;
				...
				if (this.serverStore && !this._isChildrenLoaded(parentItem)) {
					this.childrenSize = 0;
					this.store.fetch({start:start, count:count, sort:sort, query:dojo.mixin({parentId:parentId}, this.query || {}), onBegin:dojo.hitch(this, function (size) {
